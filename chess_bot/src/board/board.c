...
int cb_is_square_attacked(int square, _board *board, uint64_t occupied, _color side){
    ...
    uint64_t sliders[2];
    if (side == WHITE){
        non_sliders[0] = board->bitboards[B_PAWN];
        non_sliders[1] = board->bitboards[B_KNIGHT];
        ...
        non_sliders[1] = board->bitboards[W_KNIGHT];
    ...
    if ((pawn_attacks[side][square] & non_sliders[0]) != 0){
        return 1;
    }
    if ((knight_attacks[square] & non_sliders[1]) != 0){
        return 1;
    }
    if ((king_attacks[square] & non_sliders[2]) != 0){
        return 1;
    }

    // rook, bishop, queen attacks
    for (int i = 0; i < 8; i++){
        uint64_t ray = rays[square][i];
        int potential_attacker_square;
        if (i < 4){
            potential_attacker_square = bb_get_lsb(occupied & ray) - 1;
        }
        else{
            potential_attacker_square = bb_get_msb(occupied & ray) - 1;
        }

        if (potential_attacker_square == -1){
            continue;
        }
